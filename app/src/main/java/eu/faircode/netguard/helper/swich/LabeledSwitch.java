package eu.faircode.netguard.helper.swich;
...
public class LabeledSwitch extends ToggleableView {
    ...
    private void initProperties(AttributeSet attrs) {
        ...
        for (int i = 0; i < N; ++i) {
            ...
            if (attr == R.styleable.Toggle_on) {
            ...
            } else if (attr == R.styleable.Toggle_colorOffs) {
                colorOff = tarr.getColor(R.styleable.Toggle_colorOffs, Color.parseColor("#FFFFFF"));
                colorTextOff = colorOff;
            ...
            } else if (attr == R.styleable.Toggle_colorOns) {
                int accentColor;
                if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.M) {
                    accentColor = getResources().getColor(R.color.colorAccent, getContext().getTheme());
                } else {
                ...
                colorOn = tarr.getColor(R.styleable.Toggle_colorOns, accentColor);
                colorTextOn = colorOn;
            } else if (attr == R.styleable.Toggle_colorDisabled) {
                colorDisabled = tarr.getColor(R.styleable.Toggle_colorOffs, Color.parseColor("#D3D3D3"));
                ...
                int defaultTextSize = (int) (24f * getResources().getDisplayMetrics().scaledDensity);
            ...
            }else if(attr == R.styleable.Toggle_colorTextOn){
                colorTextOn = tarr.getColor(R.styleable.Toggle_colorTextOn, colorOn);
            }else if(attr == R.styleable.Toggle_colorTextOff){
                colorTextOff = tarr.getColor(R.styleable.Toggle_colorTextOff, colorOff);
            }
        }
...
//            int offColor = Color.RED;
            ...
            int onColor;
            if (isEnabled()) {
                onColor = Color.argb(alpha, Color.red(colorTextOn), Color.green(colorTextOn), Color.blue(colorTextOn));
//                onColor = Color.argb(alpha, Color.red(colorOn), Color.green(colorOn), Color.blue(colorOn));
            } else {
                onColor = Color.argb(alpha, Color.red(colorDisabled), Color.green(colorDisabled), Color.blue(colorDisabled));
            }
//            onColor = Color.DKGRAY;
            paint.setColor(onColor);
...
//      Drawing Switch Thumb here
        {
            ...
            int colorNew = Color.parseColor("#F7B32B");
            int alpha = (int) (((thumbBounds.centerX() - thumbOffCenterX) / (thumbOnCenterX - thumbOffCenterX)) * 255);
            alpha = (alpha < 0 ? 0 : (alpha > 255 ? 255 : alpha));
            int offColor = Color.argb(alpha, Color.red(colorNew), Color.green(colorNew), Color.blue(colorNew));
            paint.setColor(offColor);
